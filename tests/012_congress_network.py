...
class TestCongressNetwork(TestBase):
    """test pour vérifier l'envoi de paquets réseau dans congress"""
    ...
    def create_mocked_game(self, is_network=True):
        """crée une instance de jeu congress avec des mocks pour les tests
        
        params:
            is_network: indique si le jeu doit être en mode réseau
        
        retour:
            une instance de jeu congress configurée pour les tests
        """
        # ne pas vraiment créer l'instance de jeu, mais simuler une avec des mocks
        game = CongressGame.__new__(CongressGame)
    ...
    def test_chat_send_packet(self):
        ...
        CongressGame.handle_events(game, enter_event)
